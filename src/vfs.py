...
class VirtualFileSystem:
    ...
    async def read_lines(self, path: str, start_line: int = 1, max_lines: int = 100) -> List[str]:
        config, rel_path, conn_id = self._parse_path(path)
        protocol = config["protocol"]
        ...
        if protocol == "local":
            ...
            return await asyncio.to_thread(self._read_sftp, config, rel_path, start_line, max_lines)
        elif protocol == "smb":
            return await asyncio.to_thread(self._read_smb, config, rel_path, start_line, max_lines)
        return []
    ...
    async def _read_local(self, path: str, start_line: int, max_lines: int) -> List[str]:
        lines = []
        try:
    ...
    def _register_smb(self, config):
        ...
        if not username:
            ...
            smbclient.register_session(config["host"], username=username, password=password)
    ...
    def _list_smb(self, config, path, conn_id) -> List[VFSNode]:
        ...
        smb_path = fr"\\{config['host']}\{share}\{path.lstrip('/')}"
    ...
    def _read_smb(self, config, path, start_line, max_lines) -> List[str]:
        ...
        lines = []
        with smbclient.open_file(smb_path, mode='r', encoding='utf-8', errors='replace') as f:
            current = 0
            for line in f:
                current += 1
                if current >= start_line:
