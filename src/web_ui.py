...
log_searcher = LogSearcher(vfs_instance=vfs)
ai_agent = AIAgent(log_searcher=log_searcher)
...
async def search_endpoint(request: SearchRequest):
    try:
        results = await log_searcher.search_file(
            request.filepath, query=request.query, search_type=request.search_type
        )
...
async def get_directory(path: str = "./"):
    ...
    try:
        ...
        items = [
            {"name": n.name, "path": n.path, "is_dir": n.is_dir, "protocol": n.protocol}
            for n in nodes
        ]
