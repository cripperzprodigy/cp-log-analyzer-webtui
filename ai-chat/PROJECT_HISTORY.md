...
*   **Decision (Security):** Implemented a `PIIMasker` (`src/core/security.py`). It actively intercepts outbound user chat prompts and inbound log search results, masking emails, phone numbers, and credit cards.
*   **Decision (Privacy vs Visibility):** The owner specified that sysadmins running this tool need full log visibility. Therefore, PII Masking was made an optional toggle in `config.yaml` (`security.pii_masking_enabled`) and defaults to `false`.
